...
s3file.prototype.uploadFile = function (item, file, update, callback) {
	...
	var doUpload = function () {
		if ('function' === typeof field.options.filename) {
			filename = field.options.filename(item, filename, file);
		}
		var knoxClient = knox.createClient(field.s3config);
		knoxClient.putFile(file.path, path + filename, {
...
s3file.prototype.getRequestHandler = function (item, req, paths, callback) {
	...
	return function () {
		...
		if (req.files && req.files[paths.upload] && req.files[paths.upload].size) {
			if(typeof field.options.onUpload === 'function') {
				field.options.onUpload(req.files[paths.upload]);
			}
